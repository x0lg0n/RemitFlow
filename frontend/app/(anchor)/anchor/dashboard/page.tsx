      ...
      <div className="space-y-8 animate-fade-in pb-12 sm:px-0 px-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            <Button variant="secondary" size="lg" className="h-12 rounded-3xl px-6 border-(--border) hover:bg-(--surface-elevated) hover:text-(--foreground) transition-all duration-300 font-bold tracking-tight">
              <Download className="mr-2 h-4 w-4" />
              Dataset Export
            </Button>
            <Button size="lg" className="h-12 rounded-3xl px-6 bg-(--foreground) text-(--background) hover:scale-105 transition-all duration-300 font-black tracking-tighter uppercase italic group">
        ...
        {isLoading ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-48 rounded-[2.5rem] animate-pulse bg-(--surface-elevated) border border-(--border)" />
            ))}
          </div>
        ) : null}
          ...
          <>
              ...
              {[
                ...
                <div key={i} className={cn(
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div className={cn("p-4 rounded-2xl transition-transform duration-500 group-hover:scale-110 group-hover:rotate-6", item.bg, item.color)}>
                      <item.icon className="h-6 w-6" />
                    </div>
                    <div className={cn(
                      "flex items-center gap-1 px-3 py-1 rounded-full text-xs font-black italic tracking-tighter uppercase",
                      item.change >= 0 ? "bg-emerald-400/10 text-emerald-400" : "bg-rose-400/10 text-rose-400"
                    )}>
                      {item.change >= 0 ? <TrendingUp className="h-3 w-3" /> : <Activity className="h-3 w-3 rotate-180" />}
                      {item.change >= 0 ? "+" : ""}{item.change}%
                    </div>
                  </div>
                  <div className="space-y-1">
                    <p className="text-xs font-black text-(--foreground-subtle) uppercase tracking-[0.2em]">{item.label}</p>
                    <p className="text-4xl font-black text-(--foreground) tracking-tighter italic">{item.value}</p>
                  </div>
            ...
            <div className="grid gap-8 grid-cols-1 lg:grid-cols-3">
              ...
              <Card className="lg:col-span-2 rounded-[2.5rem] border border-(--border) bg-(--surface) overflow-hidden shadow-2xl">
                <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-4 sm:space-y-0 p-8">
                  <div className="space-y-1">
                    <CardTitle className="text-2xl font-black tracking-tighter uppercase italic">Institutional Growth</CardTitle>
                    <CardDescription className="text-(--foreground-muted) font-medium">14-day aggregate market movement</CardDescription>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-(--surface-elevated) border border-(--border)">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest text-(--foreground)">Volume</span>
                    </div>
                  </div>
                </CardHeader>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={data.trends}>
                        ...
                        <XAxis 
                          ...
                          tickFormatter={(value) => new Date(value).toLocaleDateString("en-US", { month: "short", day: "numeric" }).toUpperCase()}
                          dy={10}
                        />
                        <YAxis 
                          stroke="var(--foreground-muted)" 
                            ...
                            if (active && payload && payload.length) {
                              return (
                                <div className="bg-(--surface-elevated) border border-(--border) p-4 rounded-2xl shadow-2xl backdrop-blur-xl">
                                  ...
                                  <p className="text-[10px] font-bold text-(--primary) uppercase mt-1">Settlement Volume</p>
                                </div>
                              );
                            }
                            return null;
              ...
              <Card className="rounded-[2.5rem] border border-(--border) bg-(--surface) shadow-xl">
                <CardHeader className="p-8 pb-4">
                  <CardTitle className="text-2xl font-black tracking-tighter uppercase italic">Network Health</CardTitle>
                  <CardDescription className="text-(--foreground-muted) font-medium">Real-time status diagnostics</CardDescription>
                </CardHeader>
                <CardContent className="p-8 pt-4 space-y-6">
                  <div className="space-y-4">
                    {[
                      { label: "API Availability", value: "99.9%", status: "success" },
                      { label: "Settlement Speed", value: "2.4s", status: "success" },
                      { label: "Trustline Status", value: "Active", status: "success" },
                      { label: "Liquidity Score", value: "High", status: "info" },
                    ].map((item, i) => (
                      <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-(--surface-elevated)/50 border border-(--border) group transition-all duration-300 hover:border-(--primary)/20">
                        <span className="text-xs font-black uppercase tracking-widest text-(--foreground-subtle)">{item.label}</span>
                        <div className="flex items-center gap-3">
                          <span className="text-sm font-black text-(--foreground) italic tracking-tight">{item.value}</span>
                          <div className={cn(
                            "h-2 w-2 rounded-full",
                            item.status === "success" ? "bg-emerald-400 shadow-[0_0_12px_rgba(52,211,153,0.6)]" : "bg-blue-400 shadow-[0_0_12px_rgba(96,165,250,0.6)]"
                          )} />
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="pt-4">
                    <Button variant="secondary" size="lg" className="w-full h-14 rounded-2xl border-(--border) hover:bg-(--surface-elevated) hover:text-(--foreground) transition-all duration-300 font-black tracking-tighter uppercase italic">
            ...
            <Card className="rounded-[2.5rem] border border-(--border) bg-(--surface) overflow-hidden shadow-2xl">
              <CardHeader className="flex flex-row items-center justify-between p-8">
                <div className="space-y-1">
                  <CardTitle className="text-2xl font-black tracking-tighter uppercase italic">Settlement Ledger</CardTitle>
                  <CardDescription className="text-(--foreground-muted) font-medium">Comprehensive audit log of recent network activity</CardDescription>
                </div>
                <Button variant="ghost" className="text-(--primary) hover:bg-(--primary)/10 font-black tracking-tighter uppercase italic">
                  View Data Lake
                </Button>
              </CardHeader>
                  ...
                  <Table>
                    <thead className="bg-(--surface-elevated)/50">
                      <tr className="border-b border-(--border) hover:bg-transparent">
                        <TableHead className="w-56 font-black text-(--foreground-subtle) uppercase tracking-[0.2em] text-[10px] pl-8 py-5">Event Timestamp</TableHead>
                        <TableHead className="font-black text-(--foreground-subtle) uppercase tracking-[0.2em] text-[10px] py-5">Reference Hash</TableHead>
                        <TableHead className="font-black text-(--foreground-subtle) uppercase tracking-[0.2em] text-[10px] py-5">Volume</TableHead>
                        <TableHead className="font-black text-(--foreground-subtle) uppercase tracking-[0.2em] text-[10px] py-5">Protocol Fee</TableHead>
                        <TableHead className="font-black text-(--foreground-subtle) uppercase tracking-[0.2em] text-[10px] py-5">Currency Pair</TableHead>
                        <TableHead className="font-black text-(--foreground-subtle) uppercase tracking-[0.2em] text-[10px] pr-8 text-right py-5">System Status</TableHead>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-(--border)">
                      {data.recentTransactions.map((tx) => (
                        <TableRow key={tx.id} className="border-none hover:bg-(--surface-elevated)/40 transition-all duration-300">
