...
export default function DashboardPage() {
  ...
  const chartData = last7Days.map((date) => {
    const dayTotal = (transactions || [])
      .filter(
        (tx) => tx.createdAt.startsWith(date) && tx.status === "completed",
      )
  ...
  const totalVolume = (transactions || []).reduce(
    (sum, tx) => sum + (tx.status === "completed" ? Number(tx.amount) : 0),
    0,
  );
  const pendingCount = (transactions || []).filter(
    (tx) => tx.status === "pending" || tx.status === "processing",
  ).length;
  const activeAnchorCount = catalog.filter(
    (item) => item.isActiveForUser,
  ).length;
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          <Card className="border-(--border) bg-white shadow-(--shadow-md) rounded-4xl overflow-hidden group hover:shadow-(--shadow-lg) transition-all duration-500">
            <CardContent className="p-8">
              ...
              <div className="text-[10px] font-black uppercase tracking-[0.2em] text-(--foreground-subtle) mb-1">
                Total Sent
              </div>
          ...
          <Card className="border-(--border) bg-white shadow-(--shadow-md) rounded-4xl overflow-hidden group hover:shadow-(--shadow-lg) transition-all duration-500">
            <CardContent className="p-8">
              ...
              <div className="text-4xl font-black text-emerald-500 tracking-tighter mb-2">
                {txLoading ?
                  "..."
                : `$${feesSaved.toLocaleString(undefined, { maximumFractionDigits: 0 })}`
                }
              </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="space-y-8">
            <Card className="border-(--border) bg-gray-50/50 rounded-4xl overflow-hidden shadow-sm">
              ...
              <CardContent className="p-8 pt-0 space-y-3">
                <Link href="/history" className="block">
                  <Button
                    variant="ghost"
                    className="w-full justify-between h-14 rounded-2xl px-6 font-bold hover:bg-white border-2 border-transparent hover:border-(--border) group transition-all">
            ...
            <div className="p-8 rounded-4xl bg-linear-to-br from-indigo-600 to-blue-600 text-white shadow-(--shadow-glow) shadow-indigo-500/20 group hover:scale-[1.02] transition-transform duration-500">
              ...
              <h4 className="text-2xl font-black tracking-tight mb-3 leading-tight uppercase">
                Network Secure
              </h4>
              <p className="text-sm font-bold text-white/80 leading-relaxed mb-6 italic">
                All settlements verified on the Stellar ledger via SEP-31
                protocols. High velocity cross-border engine.
              </p>
          ...
          <CardHeader className="p-10 border-b border-(--border)/50">
            <div className="flex items-center justify-between">
              ...
              <Link href="/history">
                <Button
                  variant="secondary"
                  className="rounded-xl font-bold uppercase text-[10px] tracking-widest px-6 hover:bg-(--surface-elevated) transition-all">
                  View All
                </Button>
              </Link>
            </div>
          ...
          <CardContent className="p-10 pt-6">
            {txLoading ?
              <div className="space-y-4">
                {[...Array(3)].map((_, i) => (
                  <div
                    key={i}
                    className="h-20 w-full animate-pulse rounded-2xl bg-(--surface-elevated)"
                  />
                ))}
              </div>
            : (transactions || []).length === 0 ?
              <div className="text-center py-20 bg-(--surface-elevated) rounded-3xl border-2 border-dashed border-(--border)">
                ...
                <p className="text-[10px] font-black text-(--foreground-subtle) uppercase tracking-[0.2em]">
            ...
            : <div className="space-y-4">
                {(transactions || []).slice(0, 5).map((tx) => (
                  <div
                    key={tx.id}
                    className="group relative flex items-center justify-between p-6 rounded-2xl bg-(--surface-elevated) border border-(--border) transition-all hover:bg-white hover:border-blue-500/30 hover:shadow-sm">
                    <div className="flex items-center gap-5">
                      <div
                        className={`flex h-12 w-12 items-center justify-center rounded-xl font-black text-xs ${
                          tx.status === "completed" ?
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            className={`border-none font-black text-[8px] uppercase tracking-tighter px-2 h-4 ${
                              tx.status === "completed" ?
                                "bg-emerald-500/10 text-emerald-600"
                              : tx.status === "failed" ?
                                "bg-red-500/10 text-red-600"
