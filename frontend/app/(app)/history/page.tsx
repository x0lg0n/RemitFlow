...
export default function HistoryPage() {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `remitflow_history_${new Date().toISOString().split("T")[0]}.csv`,
    );
      ...
      <div className="max-w-7xl mx-auto space-y-10 py-8 px-4 animate-fade-in">
        ...
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Popover>
            ...
            <PopoverContent
              className="w-auto p-0 rounded-4xl overflow-hidden border-(--border) bg-white shadow-2xl"
              align="start">
              <Calendar
                initialFocus
                mode="range"
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="secondary"
                className="h-14 rounded-2xl border-(--border) bg-white font-black uppercase text-[10px] tracking-widest hover:bg-black hover:text-white hover:border-black transition-all duration-300 group relative w-full">
            ...
            <DropdownMenuContent
              align="end"
              className="rounded-xl border-(--border) p-2 min-w-40">
              <DropdownMenuItem
                onClick={() => setStatusFilter("all")}
                className="font-bold uppercase text-[10px] tracking-widest p-3 rounded-lg cursor-pointer">
                All Status
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => setStatusFilter("completed")}
                className="font-bold uppercase text-[10px] tracking-widest p-3 rounded-lg cursor-pointer text-emerald-600">
                Completed
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => setStatusFilter("pending")}
                className="font-bold uppercase text-[10px] tracking-widest p-3 rounded-lg cursor-pointer text-amber-600">
                Pending
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => setStatusFilter("failed")}
                className="font-bold uppercase text-[10px] tracking-widest p-3 rounded-lg cursor-pointer text-red-600">
                Failed
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          <CardContent className="p-0">
            ...
            : filteredTransactions.length === 0 ?
              <div className="text-center py-32">
                ...
                <p className="text-[10px] font-bold text-(--foreground-subtle) uppercase mt-2 tracking-widest">
                ...
                <Table>
                  <thead className="bg-gray-50/80">
                    <TableRow className="border-b border-(--border) hover:bg-transparent">
                      <TableHead className="h-14 px-8 font-black text-[10px] uppercase tracking-widest text-(--foreground-subtle)">
                        Asset / Direction
                      </TableHead>
                      <TableHead className="h-14 px-8 font-black text-[10px] uppercase tracking-widest text-(--foreground-subtle)">
                        Transaction ID
                  ...
                  <tbody>
                    {filteredTransactions.map((tx) => (
                      <TableRow
                        key={tx.id}
                        className="group border-b border-(--border)/30 hover:bg-gray-50/50 transition-all duration-300">
                        ...
                        <TableCell className="px-8">
                          <Badge
                            className={`rounded-xl border h-7 px-3 flex items-center gap-1.5 font-black text-[8px] uppercase tracking-tighter ${getStatusStyle(tx.status)}`}>
        ...
        <div className="grid gap-6 md:grid-cols-3 pt-6">
          <div className="p-8 rounded-4xl bg-(--surface-elevated) border border-(--border) flex flex-col justify-between group hover:border-black transition-all">
            ...
            <p className="text-[10px] font-bold text-(--foreground-subtle) uppercase italic">
